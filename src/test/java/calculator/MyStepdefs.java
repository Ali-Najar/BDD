...
public class MyStepdefs {
    ...
    /*
     * Important fix compared with the original laboratory example:
     * -?\d+ accepts both positive and negative integers.
     */
    ...
    public void iPressTheOperatorKey(String operator) {
        try {
            result = calculator.calculate(value1, value2, operator.charAt(0));
    ...
    public void iExpectTheResult(double expected) {
        Assert.assertNull(
                "Unexpected exception: " + (exception == null ? "" : exception.getMessage()),
                exception
        );
    ...
    public void iExpectADivisionByZeroError() {
        Assert.assertNotNull("Expected an exception, but none was thrown", exception);
